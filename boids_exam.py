...
class Config:
    ...
    SEPARATION_STEER_STRENGTH: float = 5
    ...
    ALIGNMENT_DISTANCE: int = BOID_SIZE * 100
    ALIGNEMENT_STEER_STRENGTH: float = 0.8
    ...
    COHESION_DISTANCE: int = BOID_SIZE * 50
    COHESION_STEER_STRENGTH: float = 5
...
# Q16 not implemented.
# The boids simulation still runs with the flocking behaviors.
...
class Boid:
    def __init__(self) -> None:
        ...
        speed: float = random.uniform(config.BOID_SPEED_MIN, config.BOID_SPEED_MAX)
    ...
    def _clampSpeed(self) -> None:
        ...
        if speed > config.BOID_SPEED_MAX:
            scale = config.BOID_SPEED_MAX / speed
            self.vx *= scale
            self.vy *= scale
    ...
    def _cohesion(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        center /= count
        return center - pygame.Vector2(self.x, self.y)
...
def draw_hud(screen: pygame.Surface, font: pygame.font.Font, config: Config, fps: float) -> None:
    ...
    text = f"Separation: {'ON' if config.SEPARATION_ON else 'OFF'} - Press 'S' to toggle"
    img = font.render(text, True, (255, 255, 255))
    screen.blit(img, (10, 25))

    text = f"Alignment: {'ON' if config.ALIGNEMENT_ON else 'OFF'} - Press 'A' to toggle"
    img = font.render(text, True, (255, 255, 255))
    screen.blit(img, (10, 40))

    text = f"Cohesion: {'ON' if config.COHESION_ON else 'OFF'} - Press 'C' to toggle"
    img = font.render(text, True, (255, 255, 255))
    screen.blit(img, (10, 55))

    text = f"Wall Behavior: {config.WALL_BEHAVIOR.capitalize()} - Press 'W' to toggle"
    img = font.render(text, True, (255, 255, 255))
